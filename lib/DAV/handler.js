...
var rpnTrace = require('rpn-trace');
...
(function() {
    ...
    this.invoke = function() {
        this.httpRequest._receivedAt = Date.now();
        ...
        if (jsDAV.debugMode) {
            ...
            var we = this.httpResponse.end;
            var inspect = require('eyes').inspector({styles: {all: 'magenta'}, maxLength: 20480000});
            inspect(this.httpRequest.headers, 'HEADERS '+method+' '+this.id);
            this.httpResponse.writeHead = function(code, headers) {
                //Util.log("sending header: " + code, headers);
                inspect(headers, code+' '+method+' '+self.id);
                headers['x-jsdav-request'] = self.id+' '+method+' '+self.httpRequest.url;
                this.writeHead = wh;
                this.writeHead(code, headers);
            };
            this.httpResponse.end = function(content) {
                Util.log("REQUEST END: "+ self.id +' took '+(Date.now() - self.httpRequest._receivedAt)+'ms');
            ...
            this.httpResponse.writeHead = function(code, headers) {
                rpnTrace.info(self.httpRequest.socket.remoteAddress+ " "+code+' '+method+' '+self.httpRequest.url+' took '+(Date.now() - self.httpRequest._receivedAt)+'ms');
                this.writeHead = wh;
                this.writeHead(code, headers);
            };
        }
        ...
        this.dispatchEvent("beforeMethod", method, function(stop) {
            if (stop && typeof(stop) === 'object' && stop.code) {
    ...
    this.handleError = function(e) {
        ...
        var xml = '<?xml version="1.0" encoding="utf-8"?>\n'
                ...
                + '    <a:exception>' + JSON.stringify(e) + '</a:exception>\n'
        ...
        function afterHeaders(headers) {
            var xmlContent = xml + '</d:error>';
            headers["content-type"] = "application/xml; charset=utf-8";
            headers['content-length'] = xmlContent.length;

            self.httpResponse.writeHead(code, headers);
            self.httpResponse.end(xmlContent, "utf-8");
    ...
    this.httpGet = function() {
        ...
        this.checkPreconditions(true, function(err, redirected) {
            ...
            if (redirected)
                return;
            self.server.tree.getNodeForPath(self, uri, function(err, n) {
                if (!Util.empty(err))
                    return self.handleError(err);
                node = n;
                afterCheck();
        ...
        function afterCheck() {
            ...
            self.getHTTPHeaders(uri, function(err, httpHeaders) {
                ...
                if (httpHeaders["content-length"] !== undefined) {
                ...
                if (!ignoreRangeHeader && nodeSize && range) {
                    ...
                    if (range[0]) {
                        ...
                        if (end < start) {
                            return self.handleError(new Exc.jsDAV_Exception_RequestedRangeNotSatisfiable(
                                "The end offset (" + end + ") is lower than the start offset ("
                                + start + ") siz="+nodeSize)
                            );
                        }
                        if (end > nodeSize)
                    ...
                    if (hasStream) {
                        // pipe from node to output stream and close it.
                        node.getAndPipeToStream(start, end, self.httpResponse, function(err) {
                    ...
                    if (!((since && lastModified === since) || (etag && oldEtag === etag))) {
                        ...
                        if (hasStream) {
                            node.getAndPipeToStream(null, null, self.httpResponse, function(err) {
                                if (err) return self.handleError(err);
                                self.httpResponse.end();
                            });
                        }
    ...
    this.httpHead = function() {
        ...
        this.server.tree.getNodeForPath(this, uri, function(err, node) {
            if (!Util.empty(err))
                return self.handleError(err);
        ...
        this.getRequestBody("utf8", null, function(err, data) {
            ...
            self.parsePropfindRequest(data, function(err, requestedProperties) {
                ...
                self.getPropertiesForPath(path, requestedProperties, depth, function(err, newProperties) {
                    ...
                    var body = self.generateMultiStatus(newProperties);
        ...
        this.getRequestBody("utf8", null, function(err, data) {
            ...
            self.parseProppatchRequest(data, function(err, newProperties) {
                ...
                self.updateProperties(self.getRequestUri(), newProperties, function(err, result) {
                    if (!Util.empty(err))
                        return self.handleError(err);
                    self.httpResponse.writeHead(207, {"content-type": "text/xml; charset=utf-8"});
            ...
            function afterParse() {
                ...
                self.createCollection(uri, resourceType, properties, function(err, result) {
                    ...
                    if (result && result.length) {
                        var resultBody = self.generateMultiStatus(result);
                        self.httpResponse.writeHead(207, {
                            "content-type": "text/xml; charset=utf-8",
                            'content-length': resultBody.length
                        });
                        self.httpResponse.end(resultBody);
                    }
                    else {
                        self.httpResponse.writeHead(201, {"content-length": "0"});
                        self.httpResponse.end();
            ...
            function afterDelete() {
                self.dispatchEvent("beforeUnbind", moveInfo["source"], function(stop) {
                    ...
                    self.dispatchEvent("beforeBind", moveInfo["destination"], function(stop) {
                        if (stop === true)
                            return false;
                        self.server.tree.move(self, moveInfo["source"], moveInfo["destination"], function(err) {
                            if (!Util.empty(err))
                                return self.handleError(err);
        ...
        this.getCopyAndMoveInfo(function(err, copyInfo) {
            ...
            function afterDelete() {
                self.dispatchEvent("beforeBind", copyInfo["destination"], function(stop) {
                    if (stop === true)
                        return false;
                    self.server.tree.copy(self, copyInfo["source"], copyInfo["destination"], function(err) {
                        if (!Util.empty(err))
                            return self.handleError(err);
    ...
    this.checkPreconditions = function(handleAsGET, cbprecond) {
        ...
        if (ifMatch = this.httpRequest.headers["if-match"]) {
            ...
            this.server.tree.getNodeForPath(this, uri, function(err, n) {
                if (!Util.empty(err)) {
                ...
                for (var prop in propertyMap) {
                    ...
                    if (properties[prop]) {
                        ...
                        if (properties[prop].hasFeature && properties[prop].hasFeature(jsDAV.__PROP_GETLASTMODIFIED__))
                            headers[header] = new Date(properties[prop].getTime()).toGMTString();
                        else
    ...
    this.getPropertiesForPath = function(path, propertyNames, depth, cbgetpropspath) {
        ...
        this.server.tree.getNodeForPath(this, path, function(err, parentNode) {
            if (!Util.empty(err))
    ...
    this.getHTTPRange = function() {
        ...
        return [
            matches[1] ? parseInt(matches[1]) : null,
            matches[2] ? parseInt(matches[2]) : null
        ];
    };
    ...
    this.createFile = function(uri, dataOrStream, enc, cbcreatefile) {
        ...
        this.dispatchEvent("beforeBind", uri, function(stop) {
            if (stop === true)
                return cbcreatefile();
            self.server.tree.getNodeForPath(self, dir, function(err, parent) {
                if (!Util.empty(err))
...
//                    else if (parent.createFileStream)
//                        parent.createFileStream(self, name, enc || "utf8", afterBind);
                    else
                        parent.createFile(self, name, dataOrStream, enc || "utf8", afterBind);
    ...
    this.createCollection = function(uri, resourceType, properties, cbcreatecoll) {
        ...
        this.server.tree.getNodeForPath(this, parentUri, function(err, parent) {
            if (!Util.empty(err))
                    ...
                    if (parent.hasFeature(jsDAV.__IEXTCOLLECTION__)) {
                        ...
                        parent.createDirectory(newName, function(err, res) {
                            ...
                            function rollback(exc, res) {
                                self.server.tree.getNodeForPath(self, uri, function(err, node) {
                                    if (err)
                                        return cbcreatecoll(err);
                                    self.dispatchEvent("beforeUnbind", uri, function(stop) {
                                        if (stop === true)
    ...
    this.getCopyAndMoveInfo = function(cbcopymove) {
        ...
        this.server.tree.getNodeForPath(this, destinationDir, function(err, destinationParent) {
            if (!Util.empty(err)) {
            ...
            self.server.tree.getNodeForPath(self, destination, function(err, destinationNode) {
