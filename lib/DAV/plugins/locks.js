...
(function() {
            ...
            switch (propName) {
                case "{DAV:}supportedlock" :
                    if (self.locksBackend) {
                        ...
                        self.handler.server.tree.getNodeForPath(self.handler, path, function(err, node) {
                            if (err)
    ...
    this.getLocks = function(uri, returnChildLocks, cbgetlocks) {
        ...
        Async.list(uri.split("/")).each(function(uriPart, next) {
            if (currentPath)
                currentPath += "/";
            currentPath += uriPart;
            self.handler.server.tree.getNodeForPath(self.handler, currentPath, function(err, node) {
                if (err)
                    return next((err instanceof Exc.jsDAV_Exception_FileNotFound) ? null : err);
    ...
    this.httpLock = function(e) {
        ...
        this.validateLock(uri, false, function(err, isValid, lastLock) {
            ...
            self.handler.getRequestBody("utf8", null, function(err, body) {
                if (err)
                    return e.next(err);
                ...
                if (body) {
                    ...
                    lockInfo.uri   = uri;
                    if (lastLock && lastLock.token != lockInfo.token && lockInfo.scope != jsDAV_Locks_LockInfo.SHARED)
                        return e.next(new Exc.jsDAV_Exception_ConflictingLock(lastLock));
                }
                else if (lastLock) {
                ...
                self.handler.server.tree.getNodeForPath(self.handler, uri, function(err, node) {
                    if (err) {
                    ...
                    function afterNode() {
                        ...
                        self.lockNode(uri, lockInfo, function(err) {
                            ...
                            var lockInfoContent = self.generateLockResponse(lockInfo);
                            self.handler.httpResponse.writeHead(newFile ? 201 : 200, {
                                "Content-Type": "text/xml; charset=utf-8",
                                'content-length': lockInfoContent.length,
                                "Lock-Token": "<opaquelocktoken:" + lockInfo.token + ">"
                            });
                            self.handler.httpResponse.end(lockInfoContent);
                            e.stop();
                        });
                    }
                });
    ...
    this.getTimeoutHeader = function() {
        ...
        if (header) {
            if (header.toLowerCase().indexOf("second-") === 0)
                header = parseInt(header.substr(7), 10) * 1000;
        ...
        Async.list(urls).each(function(url, next) {
            self.getLocks(url, false, function(err, aLocks) {
                ...
                Async.list(conditions).each(function(condition, next2) {
                    ...
                    // The tokens array contains arrays with 3 elements. 0=true/false
                    // for normal/not condition, 1=locktoken, 2=etag
                    ...
                    Async.list(condition["tokens"]).each(function(conditionToken, next3) {
                        ...
                        function afterKey2() {
                            ...
                            if (conditionToken[1]) {
                                lockValid = false; tokensStopped = false;
                                // Match all the locks
                    ...
                    .end(function(err) {
                        ...
                        if (!tokensStopped) {
                            ...
                            return next2(new Exc.jsDAV_Exception_LockTokenMatchesRequestUri());
    ...
    this.getIfConditions = function() {
        ...
        var uriMatch = header.match(/^<([^<>]*)>.*/);
        if (uriMatch && uriMatch.length)
            uri = uriMatch[1];

        var matchConds = header.match(/\(([^()]*)\)/g);
        matchConds = matchConds.map(function(s) { return s.replace(/[()]*/g, '') });

        for (var i=0; i < matchConds.length; i++) {
            var mc = matchConds[i];
            var not = /^Not.*/.test(mc);

            var condition = mc.match(/<(.*)>/);

            if (condition && condition.length)
                conditions.push({
                    uri: uri,
                    tokens: [ [ not ? 0 : 1, condition[1], "" ] ],
                })
        }
    ...
    this.parseLockRequest = function(xml) {
        ...
        var m = xml.match(/<[dD\:]*owner>([\w\W\n\r\t\s]*)<\/[dD\:]*owner>/i);

        lockInfo.owner = m ? m[1] : null;

        var id = Util.md5(Date.now() + '' + Math.floor(Math.random() * 1000000));
        var lockToken = "44445502-" + id.substr(0, 4) + "-" + id.substr(4, 4)
            + "-" + id.substr(8, 4) + "-" + id.substr(12, 12);

        lockInfo.token = lockToken;

        lockInfo.scope = /\<[Dd\:]*exclusive/.test(xml)
