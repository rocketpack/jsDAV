...
inspect(lockInfo, 'lockInfo')
    ...
    this.getIfConditions = function() {
		...
		var oltMatch = header.match(/\(\<(opaquelocktoken:.*)\>\)/);
		if (oltMatch && oltMatch.length) {
			conditions.push({
				uri: null,
				tokens: [ [ 1, oltMatch[1], "" ] ],
			});
        ...
        header.match(/(?:<(?:<uri>.*?)>\s)?\((?:<not>Not\s)?(?:<(?:<token>[^>]*)>)?(?:\s?)(?:\[(?:<etag>[^\]]*)\])?\)/gi,
