...
function jsDAV_Auth_Plugin(handler) {
    ...
    if (handler.server.options.authBackend)
        this.authBackend = handler.server.options.authBackend;

    // allow passing a constructor
    if (this.authBackend && typeof this.authBackend == 'function')
        this.authBackend = new handler.server.options.authBackend();
...
(function() {
    ...
    this.initialize = function() {
        this.handler.addEventListener("beforeMethod", this.beforeMethod.bind(this), Util.EventEmitter.PRIO_HIGH);
    };
